...
export async function updateInvoice(req, res) {
    try {
        ...
        const querry = isObjectIdString(id) ? { _id: id, owner: userId } : { invoiceNumber: id, owner: userId };
        const existing = await Invoice.findOne(querry);
        if (!existing) {
            return res.status(404).json({
                success: false,
                message: "Invoice not found",
        ...
        // if user changes invoice 
        // and ensure that the invoice not already exists 
        /* UPDATE */
...
export async function deleteInvoice(req, res) {
    try{
        ...
        const found = await Invoice.findOne(querry);
        if (!found) {
    }
    catch(err){
